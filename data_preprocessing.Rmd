...
ggplot(data = my_data) +
  geom_point(aes(x = funded_amnt_inv, y = funded_amnt), alpha = 0.05)
```
...
Dummy columns instead of ordered integers because we do not know whether the difference between each category will be approximately equal. Also we talked to Gwen and she suggested this was better.
```{r}
my_data <- dummy_columns(my_data, select_columns = "home_ownership", remove_selected_columns = TRUE)
...
```{r}
filter(my_data, is.na(my_data$annual_inc)) # four items are Null, drop them.
...
Alternatively, make it dummy variables.
...
There are 25 NA values

```{r}
head(my_data$inq_last_6mths)
count(raw_data, is.na(inq_last_6mths))
count(my_data, inq_last_6mths < 0)
```
Plot the range
```{r}
...
The number of inquiries in past 12 months (excluding auto and mortgage inquiries)
...
```{r}
head(my_data$inq_last_12m)
count(my_data, is.na(inq_last_12m))
count(my_data, inq_last_12m < 0)
```
Plot the range
```{r}
ggplot(data = my_data, aes(x = inq_last_12m)) +
  geom_histogram(binwidth = 1)
```
Power law distribution.

```{r}
lm.1 <- lm(int_rate~inq_last_12m, my_data)
summary(lm.1)
```
This has a significant impact on interest rate.
...
Those with no public records have higher interest rates than those who do, which I would not expect.
...
Two are na. One has a revol_util figure, so I expect this is a data error. The other has a revol_util of 0, so this may be showing a revol_bal == 0. Since it is only two observations, we should drop them if we use this column.
...
delete all columns according to "Assignment 1 - Some Remarks on Data Preprocessing" and also id and member_id
```{r}
