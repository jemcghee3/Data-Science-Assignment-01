...
# This took over two hours and did not finish.
lmProfile <- rfe(x=x, y=trainData$int_rate,
                 sizes = subsets,
                 rfeControl = ctrl)
...
Test Data
```{r}
# preProcess_test <- preProcess(data.caret.test, method=c("scale", "center", "bagImpute", "nzv")) # I don't think we need to do this again for test.
...
(MSE_mars_manual <- mean((data.caret.test$int_rate-predicted_mars_manual)^2))
```
Hooray! [1] 8.240425
...
```{r}
plot(predicted_mars,data.caret.test$int_rate)
abline (0,1)
...
```
The above took about two hours to run and produced the following: 
...
```
[1] 6.162478
[1] 6.180476 the manual model is not an exact replica of the train() model, which is odd
