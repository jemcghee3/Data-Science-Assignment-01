...
lmProfile <- rfe(x=trainData[,2:42], y=trainData$int_rate,
                 sizes = subsets,
                 rfeControl = ctrl)
...
fitted <- predict(model_lasso)

plot(model_mars, main="Model Accuracies with LASSO")

varimp_lasso <- varImp(model_lasso)
plot(varimp_mars, main="Variable Importance with LASSO")
```
...
# preProcess_test <- preProcess(data.caret.test, method=c("scale", "center", "bagImpute", "nzv")) # I don't think we need to do this again for test.
testData <- predict(preProcess_model, newdata = data.caret.test)
predicted <- predict(model_lasso, testData)
