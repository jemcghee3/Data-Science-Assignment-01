...
The caret MARS model is currently the best with a test MSE of 8.240425
...
```{r}
plot(predicted,data.caret.test$int_rate)
abline (0,1)
```
...
baggedGrid <-  expand.grid(interaction.depth = c(1, 5, 9), 
                        n.trees = (1:30)*50, 
                        shrinkage = 0.1,
                        n.minobsinnode = 20)
...
set.seed(1)
bagged.fit <- train(int_rate ~ ., data = trainData, 
                 method = "gbm", 
