...
my_data <- select(my_data, -c("annual_inc_joint", "dti_joint", "application_type_JOINT", "verification_status_joint_Source", "verification_status_joint_Verified")) 
# drop the verified status of the joint applicant due to no way to handle a partially verified application after combining

# remove outliers
my_data$annual_inc <- ifelse(my_data$annual_inc >= 1000000, my_data$annual_inc / 100, my_data$annual_inc)

# remove outliers with a z-score > 3 or < -3
# we could work on this, but trees are relatively robust against outliers

# make dummy columns factors so that preProcess does not affect them
typeof(my_data$home_ownership_MORTGAGE)
my_data$home_ownership_MORTGAGE <- as.factor(my_data$home_ownership_MORTGAGE)
my_data$home_ownership_NONE <- as.factor(my_data$home_ownership_NONE)
my_data$home_ownership_OTHER <- as.factor(my_data$home_ownership_OTHER)
my_data$home_ownership_OWN <- as.factor(my_data$home_ownership_OWN)
my_data$home_ownership_RENT <- as.factor(my_data$home_ownership_RENT)
my_data$verification_status_Source <- as.factor(my_data$verification_status_Source)
my_data$verification_status_Verified <- as.factor(my_data$verification_status_Verified)
my_data$purpose_credit_card <- as.factor(my_data$purpose_credit_card)
my_data$purpose_debt_consolidation <- as.factor(my_data$purpose_debt_consolidation)
my_data$purpose_educational <- as.factor(my_data$purpose_educational)
my_data$purpose_home_improvement <- as.factor(my_data$purpose_home_improvement)
my_data$purpose_house <- as.factor(my_data$purpose_house)
my_data$purpose_major_purchase <- as.factor(my_data$purpose_major_purchase)
my_data$purpose_medical <- as.factor(my_data$purpose_medical)
my_data$purpose_moving <- as.factor(my_data$purpose_moving)
my_data$purpose_other <- as.factor(my_data$purpose_other)
my_data$purpose_renewable_energy <- as.factor(my_data$purpose_renewable_energy)
my_data$purpose_small_business <- as.factor(my_data$purpose_small_business)
my_data$purpose_vacation <- as.factor(my_data$purpose_vacation)
my_data$purpose_wedding <- as.factor(my_data$purpose_wedding)
my_data$initial_list_status_w <- as.factor(my_data$initial_list_status_w)
...
```{r}
data.caret.train <- data.caret.train %>% dplyr::select("int_rate", everything())
preProcess_model <- preProcess(data.caret.train[, 2:57], method=c("scale", "center", "bagImpute", "nzv"))
...
```{r}
saveRDS(model_mars, "model_mars.rds")
```
...
bagEarthGrid <-  expand.grid(nprune = c(10, 20, 30, 40, 50),
                             degree = c(1, 2, 3, 6, 9, 12))
                             

# Train the model using MARS and predict on the training data itself.
model_bagmars = train(int_rate ~ ., data=trainData, method='bagEarth')
fitted_bagmars <- predict(model_bagmars)

plot(model_bagmars, main="Model Accuracies with Bagged MARS")

varimp_bagmars <- varImp(model_bagmars)
plot(varimp_bagmars, main="Variable Importance with Bagged MARS")
...
started at 12:55pm using gridTune. Still going at 16:38
Stochastic Gradient Boosting 
...
Try to build the model with preprocessing built in. This fails due to NAs. I am not sure why preProcess won't clean them.
```{r}
gbmGrid <-  expand.grid(interaction.depth = c(1, 2),
                        n.trees = c(50),
                        shrinkage = .1,
...
model_gbm <- train(int_rate ~ .,
                   data = data.caret.train,
                   method = "gbm",
                   trControl = trainControl(method = "repeatedcv", 
                                             number = 5, 
                                             repeats = 1, 
                                             verboseIter = TRUE),
                   preProcess = c("bagImpute", "scale", "center", "nzv"),
                   tuneGrid = gbmGrid,
                   verbose = 1)
model_gbm

colSums(is.na(data.caret.train))
