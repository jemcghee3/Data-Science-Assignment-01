...
levels(factor(my_data$verification_status))
...
```{r}
caret.predictors <- model.matrix(int_rate ~ ., train_transformed.knn)[,-1] #remove the first column, which is the intercept. glmnet doesn't need that
...
### Training the model on ridge regression
```{r}
predictors.Train <- model.matrix(int_rate ~ ., train_transformed.knn)[,-1] # prepare format for glmnet
...
We can use the predict() function to make predictions with ridge regression
```{r}
predictors.Test <- model.matrix(int_rate ~ ., test_transformed.knn)[,-1] # prepare format for glmnet
...
coef(m_LASSO, s = best_lambda_LASSO) # for small lambda almost all coefficients are included
coef(m_LASSO, s = cv_LASSO$lambda.1se) # We can try again with lambda.1se
coef(m_ridge.caret, s = best_lambda) # we can compare with ridge regression

```

### Training the LASSO model
```{r}
predictors.LASSO.Train <- model.matrix(int_rate ~ ., train_transformed.knn)[,-1] # prepare format for glmnet
...
We can use the predict() function to make predictions with LASSO regression
```{r}
predictors.LASSO.Test <- model.matrix(int_rate ~ ., test_transformed.knn)[,-1] # prepare format for glmnet
LASSO.pred <- predict(m_ridge.LASSO.Train, newx = predictors.LASSO.Test, s = best_lambda_LASSO) # s specifies the lambda to use
# Compute MSE just for fun - maybe to compare later with other models
