...
for (i in 1:10){
    glm.fit = glm(int_rate ~ revol_util + poly(inq_last_6mths,7) + purpose_credit_card + verification_status_Verified + poly(annual_inc,10) + poly(funded_amnt_inv,10) + purpose_debt_consolidation + `verification_status_Source Verified` + home_ownership_MORTGAGE + poly(revol_bal,3) + dti + initial_list_status_w + poly(pub_rec,i) + delinq_2yrs + purpose_home_improvement + total_pymnt + purpose_major_purchase + total_acc + open_acc + acc_now_delinq + purpose_small_business + purpose_moving + purpose_other + purpose_house + purpose_medical, data = model_data_no_na)
    cv.error.10[i] = cv.glm(model_data_no_na,glm.fit,K=5)$delta[1] # Setting K=5 means 5-fold CV
}
cv.error.10
```
...
```{r}
set.seed(1)
cv.err.glm1.5 <- cv.glm(model_data_no_na, glm1, K = 5) 
cv.err.glm2.5 <- cv.glm(model_data_no_na, glm2, K = 5) 
cv.err.glm3.5 <- cv.glm(model_data_no_na, glm3, K = 5) 
cv.err.glm4.5 <- cv.glm(model_data_no_na, glm4, K = 5) 
cv.err.glm5.5 <- cv.glm(model_data_no_na, glm5, K = 5)
cv.err.glm6.5 <- cv.glm(model_data_no_na, glm6, K = 5) 
cv.err.glm7.5 <- cv.glm(model_data_no_na, glm7, K = 5) 
cv.err.glm8.5 <- cv.glm(model_data_no_na, glm8, K = 5)
cv.err.glm9.5 <- cv.glm(model_data_no_na, glm9, K = 5)
cv.err.glm10.5 <- cv.glm(model_data_no_na, glm10, K = 5)
cv.err.glm11.5 <- cv.glm(model_data_no_na, glm11, K = 5)
cv.err.glm12.5 <- cv.glm(model_data_no_na, glm12, K = 5)
cv.err.glm13.5 <- cv.glm(model_data_no_na, glm13, K = 5)
cv.err.glm14.5 <- cv.glm(model_data_no_na, glm14, K = 5)
cv.err.glm15.5 <- cv.glm(model_data_no_na, glm15, K = 5)
cv.err.glm16.5 <- cv.glm(model_data_no_na, glm16, K = 5)
cv.err.glm17.5 <- cv.glm(model_data_no_na, glm17, K = 5)
cv.err.glm18.5 <- cv.glm(model_data_no_na, glm18, K = 5)
cv.err.glm19.5 <- cv.glm(model_data_no_na, glm19, K = 5)
cv.err.glm20.5 <- cv.glm(model_data_no_na, glm20, K = 5)
cv.err.glm21.5 <- cv.glm(model_data_no_na, glm21, K = 5)
cv.err.glm22.5 <- cv.glm(model_data_no_na, glm22, K = 5)
cv.err.glm23.5 <- cv.glm(model_data_no_na, glm23, K = 5)
cv.err.glm24.5 <- cv.glm(model_data_no_na, glm24, K = 5)
cv.err.glm25.5 <- cv.glm(model_data_no_na, glm25, K = 5)
```
