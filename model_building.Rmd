...
Chunk 1 - this loads libraries and the raw data, performs preprocessing to data
Chunk 78 - create training and test sets
Chunk 79 - preprocess data (scaling, centering, imputing, nzv) for training and test sets

From here, run the model you deside below Chunk 79. Chunk 102 is the start of our final model.
...
# the model we chose is resistant to outliers

# factorize the data
# we tried this for the dummy variables, but the outcome was much worse
...
# Non-Linear Models and Caret LASSO and Ridge Regression
...
save the test data and training data to a file
```{r}
saveRDS(data.caret.train, "dataCaretTrain.rds")
saveRDS(data.caret.test, "dataCaretTest.rds")
...
Summary of sample sizes: 446929, 446928, 446928, 446926, 446929 
Resampling results across tuning parameters:

  interaction.depth  RMSE      Rsquared   MAE     
   9                 1.575202  0.8714148  1.190052
  12                 1.522454  0.8797444  1.146662
...
Tuning parameter 'n.minobsinnode' was held constant at a
 value of 10
RMSE was used to select the optimal model using the smallest value.
The final values used for the model were n.trees = 1500, interaction.depth = 12, shrinkage =
...
```{r}
model_gbm <- readRDS("./1500treesGBM.rds")
```
...
1500 trees model: [1] 2.310146

save the model
```{r}
saveRDS(model_gbm, file = "1500treesGBM.rds")
saveRDS(model_gbm$finalModel, file = "1500treesGBM-modelOnly.rds") # "The pre-processing would not be applied to predictions that directly use the object$finalModel object." I think we need to use the other one to capture the preprocessing model.
saveRDS(data.caret.train, file = "dataCaretTrain.rds")
saveRDS(trainData, file = "trainData.rds")
saveRDS(data.caret.test, file ="dataCaretTest.rds")
saveRDS(testData, file = "testData.rds")
```

save the preprocesing model. This creates a large file and doesn't seem to stop. I let it make 500mb+ when it should be 30mb.
```{r}
...
No pre-processing
Resampling: Cross-Validated (5 fold, repeated 1 times) 
Summary of sample sizes: 446929, 446929, 446928, 446927, 446927 
Resampling results across tuning parameters:

  max_depth  nrounds  RMSE      Rsquared   MAE     
   6           50     1.893000  0.8136221  1.441961
   6           75     1.791977  0.8328613  1.362010
   6          100     1.724223  0.8452046  1.306483
   6          150     1.646863  0.8587421  1.243502
   6          200     1.597101  0.8671392  1.201478
   6          250     1.560512  0.8731636  1.170818
   6          500     1.472607  0.8871055  1.093660
   6         1000     1.420922  0.8949800  1.042966
   6         1500     1.405342  0.8973410  1.026672
   6         2000     1.400051  0.8981748  1.020104
   6         5000     1.406854  0.8974337  1.022483
   9           50     1.675656  0.8537559  1.255823
   9           75     1.621759  0.8630658  1.210725
   9          100     1.591909  0.8681133  1.185005
   9          150     1.553362  0.8745210  1.150797
   9          200     1.536086  0.8773686  1.134212
   9          250     1.524846  0.8792254  1.123421
   9          500     1.512244  0.8814518  1.108622
   9         1000     1.522010  0.8801811  1.114895
   9         1500     1.530335  0.8789985  1.122099
   9         2000     1.535236  0.8782928  1.126543
   9         5000     1.539767  0.8776443  1.130767
  12           50     1.603697  0.8661673  1.178055
  12           75     1.586330  0.8691730  1.162409
  12          100     1.579804  0.8703527  1.155873
  12          150     1.578074  0.8707902  1.153206
  12          200     1.581222  0.8703987  1.155541
  12          250     1.584817  0.8699018  1.158625
  12          500     1.599589  0.8676925  1.172526
  12         1000     1.604293  0.8669835  1.177331
  12         1500     1.604454  0.8669609  1.177479
  12         2000     1.604456  0.8669607  1.177480
  12         5000     1.604458  0.8669605  1.177481
  15           50     1.647845  0.8588671  1.195937
  15           75     1.647583  0.8590650  1.196387
  15          100     1.653633  0.8581397  1.202051
  15          150     1.662373  0.8567792  1.210596
  15          200     1.666833  0.8560807  1.215294
  15          250     1.668892  0.8557600  1.217516
  15          500     1.670708  0.8554752  1.219405
  15         1000     1.670709  0.8554752  1.219399
  15         1500     1.670709  0.8554752  1.219399
  15         2000     1.670709  0.8554752  1.219399
  15         5000     1.670709  0.8554752  1.219399
  21           50     1.726068  0.8452990  1.250364
  21           75     1.731557  0.8444096  1.256778
  21          100     1.733498  0.8440928  1.259245
  21          150     1.734219  0.8439750  1.260197
  21          200     1.734245  0.8439715  1.260250
  21          250     1.734235  0.8439735  1.260241
  21          500     1.734233  0.8439739  1.260240
  21         1000     1.734233  0.8439739  1.260240
  21         1500     1.734233  0.8439739  1.260240
  21         2000     1.734233  0.8439739  1.260240
  21         5000     1.734233  0.8439739  1.260240

Tuning parameter 'eta' was held constant at a value of 0.3
Tuning parameter 'gamma' was held constant at a value
 of 0
...
RMSE was used to select the optimal model using the smallest value.
The final values used for the model were nrounds = 2000, max_depth = 6, eta = 0.3, gamma = 0, colsample_bytree =
 1, min_child_weight = 5 and subsample = 0.6.
...
### save it
```{r}
saveRDS(model_xgb, file = "xgbModel.rds")
saveRDS(model_xgb$bestTune, file = "xgbModel_bestTune.rds") # only saves the data about what the besttune is
saveRDS(model_xgb$finalModel, file = "xgbModel_finalModel.rds")
saveRDS(preProcess_model, file = "preProcess_model.rds")
```
...
```{r}
(MSE_xgb <- mean((data.caret.test$int_rate-predicted_xgb)^2))
(MSE_xgb <- mean((testData$int_rate-predicted_xgb)^2)) 
```
...
set.seed(1)
xgbGrid <-  expand.grid(nrounds = c(1750, 1900, 2000, 2100, 2250, 2500),
...
Tuning parameter 'subsample' was held constant at a value of 0.6
```{r}
model_xgb.2$bestTune
```


### save it
```{r}
saveRDS(model_xgb.2, file = "xgbModel_2.rds")
saveRDS(model_xgb.2$finalModel, file = "xgbModel_2_finalModel.rds")
```

### test it
```{r}
predicted_xgb.2 <- predict(model_xgb.2, testData)
```

```{r}
(MSE_xgb.2 <- mean((data.caret.test$int_rate-predicted_xgb.2)^2))
(MSE_xgb <- mean((testData$int_rate-predicted_xgb.2)^2)) 
```

```{r}
plot(predicted_xgb.2,testData$int_rate)
abline (0,1)
```
